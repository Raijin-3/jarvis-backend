...
export class AdminAssessmentService {
  ...
  async getCourseModules(userToken?: string) {
    ...
    if (!coursesRes.ok) {
      throw new InternalServerErrorException(`Failed to fetch courses: ${coursesRes.status} ${await coursesRes.text()}`);
    }

    if (!subjectsRes.ok) {
      throw new InternalServerErrorException(`Failed to fetch subjects: ${subjectsRes.status} ${await subjectsRes.text()}`);
    }

    if (!modulesRes.ok) {
      throw new InternalServerErrorException(`Failed to fetch modules: ${modulesRes.status} ${await modulesRes.text()}`);
    }
    ...
    const enrichedModules = modules.map((module: any) => {
      ...
      const course = subject?.course_id ? courseMap.get(subject.course_id) : null;
  ...
  async getQuestions(filters: QueryFilters, userToken?: string) {
    const { page, limit, category_id, question_type, module_id, difficulty_level, search } = filters;
    ...
    if (moduleIds.length > 0) {
      ...
      const subjectIds = Array.from(new Set(moduleRows.map((m: any) => m.subject_id).filter(Boolean)));
      if (subjectIds.length > 0) {
        ...
        const subjectsRes = await fetch(subjectsUrl, { headers: this.headers(userToken) });
        ...
        const courseIds = Array.from(new Set(subjectRows.map((s: any) => s.course_id).filter(Boolean)));
        if (courseIds.length > 0) {
          const courseFilter = courseIds.length === 1
            ? `id=eq.${courseIds[0]}`
            : `id=in.(${courseIds.join(',')})`;
          const coursesUrl = `${this.restUrl}/courses?select=id,title&${courseFilter}`;
          const coursesRes = await fetch(coursesUrl, { headers: this.headers(userToken) });
    ...
    const enrichedQuestions = questions.map((question: any) => {
      ...
      const subject = module.subject_id ? subjectsById[module.subject_id] : null;
  ...
  async getQuestionById(id: string, userToken?: string) {
    ...
    if (question.module_id) {
      ...
      const moduleRes = await fetch(moduleUrl, { headers: this.headers(userToken) });
      ...
      if (moduleRes.ok) {
        ...
        if (module) {
          ...
          if (module.subject_id) {
            const subjectRes = await fetch(`${this.restUrl}/subjects?id=eq.${module.subject_id}&select=id,title,course_id`, { headers: this.headers(userToken) });
            if (subjectRes.ok) {
              [subject] = await subjectRes.json();
              if (subject?.course_id) {
                const courseRes = await fetch(`${this.restUrl}/courses?id=eq.${subject.course_id}&select=id,title`, { headers: this.headers(userToken) });
                if (courseRes.ok) {
  ...
  async createQuestion(questionData: CreateQuestionDto, createdBy: string, userToken?: string) {
    ...
    if ((['text', 'image_text', 'short_text', 'fill_blank'].includes(questionData.question_type)) && questionData.text_answer) {
  ...
  async seedInitialData(adminUserId: string, userToken?: string): Promise<{ message: string }> {

...
export class AdminAssessmentController {
  constructor(private readonly adminAssessmentService: AdminAssessmentService) {}
  ...
  async createCategory(@Body() createCategoryDto: CreateCategoryDto, @Req() req: any) {
    const token = (req.headers.authorization as string | undefined)?.replace(/^Bearer\s+/i, '');
    return this.adminAssessmentService.createCategory(createCategoryDto, req.user.id, token);
  }
  ...
  async updateCategory(
    ...
    const token = (req.headers.authorization as string | undefined)?.replace(/^Bearer\s+/i, '');
    return this.adminAssessmentService.updateCategory(id, updateCategoryDto, token);
  }
  ...
  @Post('questions')
  async createQuestion(@Body() createQuestionDto: CreateQuestionDto, @Req() req: any) {
    const token = (req.headers.authorization as string | undefined)?.replace(/^Bearer\s+/i, '');
    
    // Validate question data
    this.validateQuestionData(createQuestionDto);
    
    return this.adminAssessmentService.createQuestion(createQuestionDto, req.user.id, token);
  }
  ...
  async updateQuestion(
    ...
    const token = (req.headers.authorization as string | undefined)?.replace(/^Bearer\s+/i, '');
    return this.adminAssessmentService.updateQuestion(id, updateQuestionDto, token);
  }
  ...
  async bulkCreateQuestions(
    ...
    return this.adminAssessmentService.bulkCreateQuestions(questions, req.user.id, token);
  }
  ...
  async uploadImage(@UploadedFile() file: Express.Multer.File, @Req() req: any) {
    ...
    const token = (req.headers.authorization as string | undefined)?.replace(/^Bearer\s+/i, '');
    return this.adminAssessmentService.uploadImage(file, req.user.id, token);
  }
  ...
  async createTemplate(@Body() createTemplateDto: CreateTemplateDto, @Req() req: any) {
    ...
    return this.adminAssessmentService.createTemplate(createTemplateDto, req.user.id, token);
  }
  ...
  async updateTemplate(
    ...
    const token = (req.headers.authorization as string | undefined)?.replace(/^Bearer\s+/i, '');
    return this.adminAssessmentService.updateTemplate(id, updateTemplateDto, token);
  }
  ...
  private validateTemplateData(templateDto: CreateTemplateDto): void {
    ...
    if (templateDto.passing_percentage < 0 || templateDto.passing_percentage > 100) {
    ...
    if (templateDto.difficulty_distribution) {
      ...
      if (easy + medium + hard !== 100) {
        throw new BadRequestException('Difficulty distribution percentages must sum to 100');
      }
    }
  }
}
